...
class CFExplainer(ExplainerBase):
    ...
    def __loss__(self, raw_preds: Tensor, x_label: Union[Tensor, int]):
        ...
        if self.explain_graph:
            loss = relu(raw_preds[:, x_label]).sum()
        else:
            loss = relu(raw_preds[self.node_idx, x_label]).sum()

        m = self.edge_mask.sigmoid()
        
        if self.L1_dist:
            edge_dist_loss = torch.linalg.norm(1 - m, ord=1)
        else:
            edge_dist_loss = torch.nn.functional.binary_cross_entropy(m, torch.ones_like(m).to(m.device))
        
        loss = self.alpha * loss + (1 - self.alpha) * edge_dist_loss
