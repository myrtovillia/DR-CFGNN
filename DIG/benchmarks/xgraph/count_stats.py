...
subfolders = [ "MELD_msg", "MELD_msg_link_prediction", "MELD_whole_graph"] # their order is important for if sub == "MELD_msg_link_prediction" and number in seen_numbers:
...
for sub in subfolders:
    ...
    for fname in os.listdir(folder_path):
        if fname.endswith(".pt"):
            number = int(fname.split("_")[0])
            if sub == "MELD_msg_link_prediction" and number in seen_numbers:
                continue
            numbers.add(number)
...
print(" ")
print(results)
print(f"Total number of distinct counterfactuals: {len(seen_numbers)}")
print(" ")
