...
def pipeline(config):
    ...
    for f in os.listdir(explanation_saving_dir):
    	if f.endswith('.pt'):

    		if not "_DENOISED" in f:
...
#----------------------------------------------------------------------------------------------------------------
    		...
    		if reconstruction_deconstruction:
    			...
    			nx.draw_networkx_edges(G_whole, pos, edgelist=removed_edges, edge_color='red', width=2.5)
    			if hasattr(data, 'smiles'):
