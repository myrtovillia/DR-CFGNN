...
def convert_ba2motifs_to_three_classes(data_list, motif_nodes=[20, 21, 22, 23, 24], seed=42):
    import random
    random.seed(seed)
    torch.manual_seed(seed)
    ...
    idx_0 = random.sample(range(len(class_0)), n_0)
    idx_1 = random.sample(range(len(class_1)), n_1)

    def remove_one_edge(data, class_id, motif_nodes=[20, 21, 22, 23, 24]):		
	    print(class_id)
	    motif_nodes_tensor = torch.tensor(motif_nodes, dtype=torch.long)
	    print(motif_nodes_tensor)
	    edge_index = data.edge_index
	    print(edge_index)
	    src, dst = edge_index[0], edge_index[1]
	    motif_edge_mask = torch.isin(src, motif_nodes_tensor) & torch.isin(dst, motif_nodes_tensor)
	    print(motif_edge_mask)
	    motif_edge_indices = motif_edge_mask.nonzero(as_tuple=False).view(-1)
	    ...
	    if class_id == 1:
	    	...
	    	protected_edges = []
	    undirected_edges = [edge for edge in undirected_edges if edge not in protected_edges]
	    print(undirected_edges)
	    sampled_edges = random.sample(undirected_edges, 1)
	    print(sampled_edges)

	    
	    to_remove_set = set()
	    for (u, v) in sampled_edges:
	    	to_remove_set.add((u, v))
	    	to_remove_set.add((v, u))
	    print(to_remove_set)
	   
	    edge_pairs = list(zip(src.tolist(), dst.tolist()))
	    print(edge_pairs)
	    keep_mask = torch.tensor([(u, v) not in to_remove_set for (u, v) in edge_pairs], dtype=torch.bool)
	    print(keep_mask)
    ...
    for i in idx_0:
        class_0[i] = remove_one_edge(class_0[i], class_id=0)
        class_0[i].y = torch.tensor([2])      
    for i in idx_1:
        class_1[i] = remove_one_edge(class_1[i], class_id=1)
        class_1[i].y = torch.tensor([2])
